basehost = typeof basehost=='undefined'?'index.php?':basehost;
(function( $ ){
    // 当domReady的时候开始初始化
    $(function() {
        var $wrap = $('#uploader'),
        ...
        uploader = WebUploader.create({
            ...
            fileNumLimit: 10,	//设置添加文件的数量
            fileSizeLimit: 200 * 1024 * 1024,    // 设置添加文件的总大小，当前200M
            fileSingleSizeLimit: 50 * 1024 * 1024    // 设置添加单个文件的大小，当前50 M
        });
        // 拖拽时不接受 js, txt 文件。
        ...
        uploader.on( 'uploadSuccess', function( file, response ) {
            if(response._raw.indexOf('上传失败')>=0){
                if(failmsg){
                    alert(response._raw);
                    failmsg = false;
                }
                ...
                $('li#'+file.id).append(
                    '<div class="file-panel" style="overflow: hidden; height: 0px;"><span class="cancel" onclick="DelAlbPic(\''+file.id+'\')">删除</span></div><span class="success"></span>'
        ...
        function addFile( file ) {
            ...
            if(_setText != ''){
                html+='<div class="formWrap"><input type="text" name="picinfook'+file.id+'" class="am-form-field" placeholder="图片未命名" /></div>';
            }
            html +='</li>';   
            failmsg = true;
            var $li = $(html),
...
$(function(){
    ...
    $(".libtn").mouseover(function(){
        var index = $(".libtn").index($(this));
        $($(".btn_photo").get(index)).addClass("visible");
    });
    $(".libtn").mouseout(function()
    {
        var index = $(".libtn").index($(this));
        $($(".btn_photo").get(index)).removeClass("visible");
    });
})