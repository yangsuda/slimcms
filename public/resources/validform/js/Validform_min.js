...
(function(d, f, b) {
	...
	var e = {
		...
		w: {
			...
			"qq": "QQ号码格式不对！",
			"url": "请填写网址！",
			"color_hex": "请设置颜色，如#FF0000",
			"color_rgba": "请设置颜色，如rgba(255,255,255,1)",
			"money2": "请填写价格，允许2位小数！",
			"money3": "请填写价格，允许3位小数！",
			"number_2": "限数字，不为0，允许2位小数！",
			"identity":"身份证格式不正确",
			"max2": "最多选2个！",
			"max3": "最多选3个！",
			"max4": "最多选4个！",
	...
	a.util = {
		dataType: {
			...
			tel: /\d{3}-\d{8}$|\d{4}-\d{7}$/,
			banktel: /^95[0-9]{3}$|96[0-9]{3}$|96[0-9]{4}$/,
			tel400:/^400[0-9]{7}$/,
			tel800:/^800[0-9]{7}$/,
			e: /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
			qq: /^[1-9]\d{4,14}$/,
			...
			color_rgba: /^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*([.\d]+))?\)$/,
			money2: /^(0|[1-9][0-9]{0,9})(\.[0-9]{1,2})?$/,
			money3: /^(0|[1-9][0-9]{0,9})(\.[0-9]{1,3})?$/,
			number_2: /^([1-9][0-9]*)+(.[0-9]{1,2})?$/,
			"identity":function(cur){
				return IdentityCodeValid(cur);
			},
			...
			"max3": function(gets,obj,curform,regxp){
				var atmax=3,
					numselected=curform.find("input[name='"+obj.attr("name")+"']:checked").length;
			...
			"max4": function(gets,obj,curform,regxp){
				var atmax=4,
					numselected=curform.find("input[name='"+obj.attr("name")+"']:checked").length;
			...
			"max5": function(gets,obj,curform,regxp){
				var atmax=5,
					numselected=curform.find("input[name='"+obj.attr("name")+"']:checked").length;
		...
		showmsg: function(n, l, m, k) {
			...
			if (l == 1 || k == "byajax" && l != 4 && l != 'wap') {
				j.find(".Validform_info").html(n)
			}
			if (l == 1 && k != "bycheck" && m.type != 2 || k == "byajax" && l != 4 && l != 'wap') {
				i = false;
				j.find(".iframe").css("height", j.outerHeight());
				j.show();
				h(j, 100)
			}
			if(l == 'wap' && m.type != 1 || k == "byajax" && l != 4 && m.type != 1){
				j.find(".Validform_checktip").html(n);
				a.util.cssctl(j.find(".Validform_checktip"), m.type)
			}
			if (l == 'wap' && k != "bycheck" && m.type != 2 && m.type != 1 || k == "byajax" && l != 4 && m.type != 1) {
				i = false;
				j.css('bottom','50%');
				setTimeout(function() {
					j.css('bottom','-100px');
		...
		check: function(v, t, n) {
			...
			if (!s.passed) {
				...
				if (!n) {
					...
					!o.tipSweep && r.addClass("Validform_error").addClass("is-invalid").removeClass('is-valid')
				}
				return false
			}
				...
				var p = {
					...
					success: function(x) {
						if (d.trim(x.status) === "y") {
							...
							r.removeClass("Validform_error").removeClass("is-invalid").addClass('is-valid');
							g = null;
							...
							r.addClass("Validform_error").addClass("is-invalid").removeClass('is-valid')
						}
		...
		submitForm: function(o, l, k, r, t) {
			...
			w.find("[datatype]").each(function() {
				...
				if (!n.passed) {
					...
					A.addClass("Validform_error").addClass("is-invalid").removeClass('is-valid');
					if (!o.showAllError) {
						A.focus();
						s = false;
						return false
				...
				A.removeClass("Validform_error").removeClass("is-invalid").addClass('is-valid');
				g = null
			});
			if (o.showAllError) {
		...
		resetForm: function() {
			...
			k.find(".Validform_error").removeClass("Validform_error").removeClass("is-invalid").addClass('is-valid');
	...
	a.prototype = {
		...
		ignore: function(k) {
			...
			d(l.forms).find(k).each(function() {
				d(this).data("dataIgnore", "dataIgnore").removeClass("Validform_error").removeClass("is-invalid").addClass('is-valid')
			});
			return this
		},
		unignore: function(k) {
		...
		ajaxPost: function(k, m, l) {
			...
			d(n.forms).each(function() {
				if (this.settings.tiptype == 1 || this.settings.tiptype == 2 || this.settings.tiptype == 3) {
					c()
				}else if(this.settings.tiptype == 'wap'){
					cc();
				}
	...
	function c() {
		...
		j.find("a.Validform_close").click(function() {
			...
			i = true;
			if (g) {
				g.focus().addClass("Validform_error").addClass("is-invalid").removeClass('is-valid')
			}
			return false
		}).focus(function() {
			this.blur()
	...
	function cc() {
		if (d(".Validform_alert").length !== 0) {
			return false
		}
		j = d('<div class="Validform_alert"><div id="msgvalid" class="Validform_checktip"></div></div>').appendTo("body");
		j.find(".Validform_checktip").click(function() {
			j.css({"bottom":"-100px"});
			i = true;
			if (g) {
				g.focus().addClass("Validform_error").addClass("is-invalid").removeClass('is-valid');
